...
module ActivityPub
  class ActorFetcher
    ...
    def fetch_actor_data(actor_uri)
      ...
      raise ActorFetchError, "Invalid JSON response: #{e.message}"
    rescue Net::TimeoutError => e
      raise ActorFetchError, "Request timeout: #{e.message}"
    end
    ...
    def create_actor_from_data(actor_uri, actor_data)
      ...
      raise ActorFetchError, "Database error: #{e.message}"
    end
    ...
    def parse_actor_response(response)
      raise ActorFetchError, "HTTP #{response.code}: #{response.message}" unless response.success?

      JSON.parse(response.body)
    end

    def validate_actor_data(actor_data)
      raise ActorFetchError, "Invalid actor type: #{actor_data['type']}" unless actor_data['type']&.match?(/Person|Service|Organization|Group/)

      required_fields = %w[id inbox outbox publicKey]
      missing_fields = required_fields.select { |field| actor_data[field].blank? }

      raise ActorFetchError, "Missing required fields: #{missing_fields.join(', ')}" if missing_fields.any?
    end
    ...
    def extract_public_key(actor_data)
      ...
      raise ActorFetchError, 'Missing public key in actor data' unless public_key_pem
