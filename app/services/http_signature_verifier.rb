...
module ActivityPub
  class HttpSignatureVerifier
    ...
    def parse_signature_header
      ...
      raise SignatureError, 'Missing Signature header' unless signature_header
      ...
      raise SignatureError, "Missing signature parameters: #{missing.join(', ')}" if missing.any?
    ...
    def fetch_actor_public_key(actor_uri)
      ...
      raise SignatureError, 'No public key found in actor data' unless public_key_data
    ...
    def fetch_actor_data(actor_uri)
      ...
      raise SignatureError, "Invalid JSON in actor response: #{e.message}"
    rescue StandardError => e
      raise SignatureError, "Network error fetching actor: #{e.message}"
    end
    ...
    def parse_public_key(public_key_pem)
      ...
      raise SignatureError, "Invalid public key format: #{e.message}"
    end
    ...
    def validate_response(response)
      raise SignatureError, "Failed to fetch actor: HTTP #{response.code}" unless response.code == '200'
    end
